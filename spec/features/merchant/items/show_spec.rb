  ...
  describe "editing merchant items" do
    it "links item name text to item show page" do
      ...
      within "#merchant_items" do
        ...
        expect(page).to have_link("#{@item_1.name}", href: edit_merchant_item_path(@item_1))
        
        # When I click the link
        click_link "#{@item_1.name}"
        
        # Then I am taken to a page to edit this item
        expect(current_path).to eq(edit_merchant_item(@item_1))
        
        # And I see a form filled in with the existing item attribute information
        expect(find_field('Name:').value).to eq(@item_1.name)
        expect(find_field('Description:').value).to eq(@item_1.description)
        expect(find_field('Unit Price:').value).to eq(@item_1.price)
