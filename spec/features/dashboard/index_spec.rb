    ...
    describe "when I visit my merchant dashboard," do
      ...
      it "and I see the names of the 5 customers with whom I have the most completed transactions, as well as the number of each customer's successful transactions." do
        ...
        within "#top_customers" do
          expect(page).to have_content("#{@customer_6.first_name} #{@customer_6.last_name} - #{Customer.top_five_customers[0].purchases} purchases")
          expect(page).to have_content("#{@customer_5.first_name} #{@customer_5.last_name} - #{Customer.top_five_customers[0].purchases} purchases")
          expect(page).to have_content("#{@customer_4.first_name} #{@customer_4.last_name} - #{Customer.top_five_customers[0].purchases} purchases")
          expect(page).to have_content("#{@customer_3.first_name} #{@customer_3.last_name} - #{Customer.top_five_customers[0].purchases} purchases")
          expect(page).to have_content("#{@customer_2.first_name} #{@customer_2.last_name} - #{Customer.top_five_customers[0].purchases} purchases")
          
          expect(page).to_not have_content("#{@customer_1.first_name} #{@customer_1.last_name}: 0")
        end
      end
    end
  end
